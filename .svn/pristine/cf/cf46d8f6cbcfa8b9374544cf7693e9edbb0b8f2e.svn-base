...
DrawioFile.prototype.addUnsavedStatus = function (err) {
	if (!this.inConflictState && this.ui.statusContainer != null && this.ui.getCurrentFile() == this) {
		...
		// 		/**
		// 		 * xhw 2022-04-18
		// 		 * 移除drawio自带保存功能，增加ctc保存功能
		// 		 * !this.isCompressed() true 压缩  false不压缩
		// 		 */
		// 		mxEvent.addListener(links[0], 'click', mxUtils.bind(this, function () {
		...
		// 					if(item.type!=='link'&&item.value.indexOf('div')===-1&&!item.getName()) model.setName(item,item.value)
							
		// 				})
		// 				model.endUpdate()
		// 				graph.cellEditor.stopEditing(false)
		// 				graph.refresh()
		// 				const currentFile = this.ui.getFileData(null, null, null, null,
		// 					null, null, null, null, this, this.isCompressed());
        //                     console.log("🚀 ~ file: DrawioFile.js ~ line 1525 ~ this", this)
