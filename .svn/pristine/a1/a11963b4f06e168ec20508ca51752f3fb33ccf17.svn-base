...
window.VSD_CONVERT_URL =
  window.VSD_CONVERT_URL ||
  "https://convert.diagrams.net/VsdConverter/api/converter";
window.EMF_CONVERT_URL =
  window.EMF_CONVERT_URL || "https://convert.diagrams.net/emf2png/convertEMF";
...
window.DRAWIO_GITLAB_ID =
  window.DRAWIO_GITLAB_ID ||
  "2b14debc5feeb18ba65358d863ec870e4cc9294b28c3c941cb3014eb4af9a9b4";
window.DRAWIO_GITHUB_URL = window.DRAWIO_GITHUB_URL || "https://github.com";
window.DRAWIO_GITHUB_API_URL =
  window.DRAWIO_GITHUB_API_URL || "https://api.github.com";
...
window.NOTIFICATIONS_URL =
  window.NOTIFICATIONS_URL || "https://www.draw.io/notifications";
window.RT_WEBSOCKET_URL =
  window.RT_WEBSOCKET_URL || "wss://app.diagrams.net/rt";
...
window.ICONSEARCH_PATH =
  ...
  window.location.protocol != "file:"
    ? "iconSearch"
    : window.DRAWIO_BASE_URL + "/iconSearch");
...
if (
  urlParams["extAuth"] == "1" &&
  /((iPhone|iPod|iPad).*AppleWebKit(?!.*Version)|; wv)/i.test(
    navigator.userAgent
  )
) {
  urlParams["gapi"] = "0";
...
if (
  ...
  DRAWIO_LIGHTBOX_URL.substring(DRAWIO_LIGHTBOX_URL.indexOf("//") + 2)
) {
  urlParams["lightbox"] = "1";
}
...
(function () {
  ...
  if (typeof JSON !== "undefined") {
    ...
    if (isLocalStorage) {
      try {
        var key =
          urlParams["sketch"] == "1" ? ".sketch-config" : ".drawio-config";
  ...
  if (glUrl != null) {
    ...
    if (
      glUrl.substring(0, 7) != "http://" &&
      glUrl.substring(0, 8) != "https://"
    ) {
      glUrl = "http://" + glUrl;
    }
...
if (window.location.hostname == "se.diagrams.net") {
  ...
  window.DRAWIO_GOOGLE_CLIENT_ID =
    window.DRAWIO_GOOGLE_CLIENT_ID ||
    "184079235871-pjf5nn0lff27lk8qf0770gmffiv9gt61.apps.googleusercontent.com";
}
