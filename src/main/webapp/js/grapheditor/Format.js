...
StyleFormatPanel.prototype.addEditOps = function (div) {
	...
	if (ss.cells.length == 1) {
		...
		// xhw
		btn = mxUtils.button(mxResources.get('editProcess'), mxUtils.bind(this, function (evt) {
			this.editorUi.actions.get('editProcess').funct();
		}));

		btn.setAttribute('title', mxResources.get('editProcess') + ' (' + this.editorUi.actions.get('editProcess').shortcut + ')');
		btn.style.width = '210px';
		btn.style.marginBottom = '2px';
