...
Menus.prototype.init = function () {
  ...
  this.put(
    ...
    new Menu(
      mxUtils.bind(this, function (menu, parent) {
        ...
        ui.editor.setStatus('');
        window.top['drawio']['saveHistory'](this.editorUi.getFileData(
          null,
          null,
          null,
          null,
  ...
  const check = (publish) => {
    ...
    const beforeCheck = graph.getSelectionCells()
    graph.selectAll(null, true);
    const all = mxClipboard.copy(graph);
    const model = graph.getModel();
    model.beginUpdate();
    const checkType = (cell) => {
      if (cell.style.indexOf('fillColor=#d9d9d9') !== -1) return 'quote_transition';
    ...
    let wrongArr;
    let errorMessage = '';
    let start = all.filter(item => item.type === 'start')
    let sourceTargetArr = []
    edges.forEach((item) => {
      sourceTargetArr.push([item.source, item.target])
    })
    for(let item of sourceTargetArr) {
      let times = 0
      for(let i = 0; i < sourceTargetArr.length; i++){
        if(item[0].id===sourceTargetArr[i][1].id && item[1].id===sourceTargetArr[i][0].id) {
          graph.setSelectionCells(item)
          window.top['drawio']['error'](mxResources.get('cantLinkEach'))
          return
        }else if(item[0].id===sourceTargetArr[i][0].id && item[1].id===sourceTargetArr[i][1].id ) {
          times++
          if(times === 2){
            graph.setSelectionCells(item)
            window.top['drawio']['error'](mxResources.get('cantLinkSame'))
            return
          }
        }
      }
    ...
    if (transitionSource.length < transitionId.length) {
      ...
      wrongArr.forEach((item) => {
        graph.setSelectionCell(transitions.find((findItem) => findItem.id === item));
    ...
    if (gatewaysId.length) {
      ...
      if (gatewaysSource.length < gatewaysId.length) {
        ...
        window.top['drawio']['error'](errorMessage);
        return;
      }else if(gatewaysSourceId.length > gatewaysId.length) {
        wrongArr = getDoubleAppear(gatewaysSourceId)
    ...
    if (gatewayImplicitId.length) {
      let gatewaysSource = edges.filter((item) => {
        return gatewayImplicitId.includes(item.target.id);
      });
      let gatewaysSourceId = gatewaysSource.map((item) => {
        return item.target.id;
      });
      if (gatewaysSource.length < gatewayImplicitId.length) {
        wrongArr = getArrDifference(gatewayImplicitId, gatewaysSourceId);
        wrongArr.forEach((item) => {
          let name = gatewayImplicits.find((findItem) => {
            if (findItem.id === item) {
              graph.setSelectionCell(findItem);
            }
            return findItem.id === item;
      ...
      if (gatewaysTarget.length < 2 * gatewayImplicitId.length) {
        wrongArr = getArrDifference(gatewayImplicitId, gatewaysTargetId).concat(
          getNotDoubleAppear(gatewaysTarget)
        );
        wrongArr.forEach((item) => {
          let name;
          if (
            gatewayImplicits.find((findItem) => {
              if (findItem.id === item) {
              }
              return findItem.id === item;
            })
          ) {
            name = gatewayImplicits.find((findItem) => {
              if (findItem.id === item) {
                graph.setSelectionCell(findItem);
              }
              return findItem.id === item;
    ...
    if (gatewayAndsplitId.length) {
      let gatewaysSource = edges.filter((item) => {
        return gatewayAndsplitId.includes(item.target.id);
      });
      let gatewaysSourceId = gatewaysSource.map((item) => {
        return item.target.id;
      });
      if (gatewaysSource.length < gatewayAndsplitId.length) {
        wrongArr = getArrDifference(gatewayAndsplitId, gatewaysSourceId);
        wrongArr.forEach((item) => {
          let name = gatewayAndsplit.find((findItem) => {
            if (findItem.id === item) {
              graph.setSelectionCell(findItem);
            }
            return findItem.id === item;
        ...
        window.top['drawio']['error'](errorMessage);
        return;
      }else if(gatewaysSourceId.length > gatewayAndsplitId.length) {
        wrongArr = getDoubleAppear(gatewaysSourceId)
        wrongArr = [...new Set(wrongArr)];
        wrongArr.forEach((item) => {
          let name = gatewayAndsplit.find((findItem) => {
      ...
      if (gatewaysTarget.length < 2 * gatewayAndsplitId.length) {
        wrongArr = getArrDifference(gatewayAndsplitId, gatewaysTargetId).concat(
          getNotDoubleAppear(gatewaysTarget)
        );
        wrongArr.forEach((item) => {
          let name;
          if (
            gatewayAndsplit.find((findItem) => {
              if (findItem.id === item) {
              }
              return findItem.id === item;
            })
          ) {
            name = gatewayAndsplit.find((findItem) => {
              if (findItem.id === item) {
                graph.setSelectionCell(findItem);
              }
              return findItem.id === item;
    ...
    if (errorMessage) {
      ...
      if(beforeCheck){
        graph.setSelectionCells(beforeCheck)
      }else{
        graph.clearSelection()
      }
      
      if(!publish) window.top['drawio']['success']();
      return true;
    }
  };
  this.put(
