...
Actions.prototype.init = function () {
	...
	function deleteCells (includeEdges) {
		...
		graph.escape();
		//删除节点时,调用接口将数据库中的节点信息删除 llz
		//取消该功能,因为用户可能会撤销操作,需要在保存时再进行比对 llz
		// window.top['drawio']['delete'](graph.getDeletableCells(graph.getSelectionCells())[0].id)
		const deleteAll = graph.getSelectionCells()
		try{
			deleteAll.forEach(deleteOne =>{
				if(deleteOne.type ==='user_transition'){
					graph.selectAll()
					const all = mxClipboard.copy(graph);
					all.forEach(item=>{
						if(item.type ==='quote_transition' && item.quoteId === deleteOne.id){
							window.top['drawio']['infoToast'](mxResources.get("cantDeleTransition"))
							graph.setSelectionCell(deleteOne);
							throw Error()
						}
					})
				}
		...
		var select = graph.deleteCells(graph.getDeletableCells(deleteAll), includeEdges);
	...
	//复用的实现函数 llz
	this.addAction('myReuse', function () {
		try {
			//todo 获取Parent里的线,确定是否连着东西
			// const children = mxClipboard.cells
			const all = mxClipboard.copy(graph)
			const children = all[0].parent.children
			let edges = children.filter(item=>{
				return item.edge && item.target.id === all[0].id && (item.source.value ==='并行合并'||item.source.value ==='后置排他拆分')
			})
			if(edges.length){
				window.top['drawio']['error'](`${edges[0].source.value}`+ mxResources.get("cantReuse"))
				throw Error()
			}
			mxClipboard.cells[0].setQuoteId(all[0].id)
			mxClipboard.cells[0].setType('quote_transition')
			ui.reUseXml();
		}
		catch (e) {
			return
		...
		try { 
			//修改样式变成灰色 llz
			var style = mxClipboard.cells[0].getStyle()
			mxClipboard.cells[0].setStyle(style+"fillColor=#d9d9d9")
		}
		catch (e) {
			ui.handleError(e);
		}
	 ...
	 * 操作父页面，打开编辑流程
	 */
	this.addAction('editProcess...', mxUtils.bind(this, function () {
	...
	//初始化的时候生成缩略图 llz   
	setTimeout(()=>{
		this.outlineWindow = new OutlineWindow(ui, document.body.offsetWidth - 260, 700, 180, 180);
	})
